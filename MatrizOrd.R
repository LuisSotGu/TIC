...
#Carga de el shp
shape_prov<-st_read("C:\\Users\\sotom\\Desktop\\TIC\\SHP\\nxprovincias.shp")
shape_prov <- shape_prov %>%
  filter(!(DPA_DESPRO %in% c("GALAPAGOS", "ZONA NO DELIMITADA")))

ingresos_ecuador_compilado <- read_csv("C:\\Users\\sotom\\Desktop\\TIC\\ingresos_ecuador_compilado.csv")
#Ingresos 
...
############### Manipulacion de la matriz W_ord ################################

W_ord_sf <- mat2listw(W_ord,style = "W") #Transforma la matriz W_ord en un objeto compatible con sp

vecinos <- vector("list", nrow(W_ord))

for(i in 1:length(vecinos)){
  vecinos[i] <- as.list(W_ord_sf$neighbours[1])
}

# Nuevo objeto con nuevos pesos y vecinos

nbs_ord<- shape_prov |> mutate(
  wt = W_ord_sf$weights,
  nb = vecinos
)
...
getis_stat<- nbs_ord |> 
  mutate(
    Gi = local_gstar_perm(ing, nb,wt, nsim = 999)
    # nsim = numero de simulaciones Monte Carlo 
  ) |> 
  unnest(Gi)
